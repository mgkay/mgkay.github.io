...
% Matlog Version 17 01-Mar-2017 (http://www.ise.ncsu.edu/kay/matlog)
...
if nargin < 4, doLTL = false; end
   % Handle "Function fails on array inputs" warning in 2016b
if nargout < 2 && length(q(:)) > 1 && length(sh(:)) == 1
   sh(1:length(q(:))) = sh;
end
