...
% Matlog Version 17 09-Feb-2018 (http://www.ise.ncsu.edu/kay/matlog)
...
if isempty(loc_h), loc_h = @(W,X0) minisumloc(P,W,p,[],X0); end
% End (Input Error Checking) **********************************************

if ~isempty(P) && size(P,2) == 2, alaplot(X,[],P,'Locate'); end
...
while ~done
   ...
   if ~isempty(P) && size(P,2) == 2, alaplot(X,W1,P,'Allocate'); end
   ...
   if TC > TC1
      ...
      if ~isempty(P) && size(P,2) == 2, alaplot(X,W,P,'Locate'); end
   else
      done = 1;
   end
end
...
while ~done
   ...
   idx0 = find(all(W==0,2));  % Ck for unallocated NFs
   if isempty(idx0)
      done = true;
   else  % Randomly relocate unallocated NFs to EFs
      idxP = randperm(size(P,1));
      X(idx0,:) = P(idxP(1:length(idx0)),:);
   end
end
TC = sum(sum(W.*D));
