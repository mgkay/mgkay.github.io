...
- **`/tc edits`** (a subcommand of `track-changes` itself, added 9.9.0) — the other direction of review: instead of dictating corrections, **you edit the document directly** and this reports what you changed. It diffs the file against an automatic snapshot of *the file as the AI last left it* (captured by the PostToolUse hook on every AI write, `/draft` writes included, so git's inability to separate your edits from the AI's in one dirty tree never arises), then reports the edited line spans, `tc-polish`'s scope over just those spans, your **project's own linter's** findings inside them (configured in a repo-local `.tc-edits.json` — the skill supplies the mechanism, your project supplies the rules), any tracked region an edit overlaps, and the next free mark number. It **never edits the document**: your text stays clean and only the AI's corrections to it become marks. The snapshot store doubles as an undo buffer (`/tc edits restore`). No new hook, no new skill directory — it ships in `track-changes/lib/tc_edits.py` and is dispatched by `/tc` the way `/tc source` and `/tc manifest` are.

When Claude Code follows the instructions below, it downloads all three skills — hooks, library modules, the shared `tc_core` package, slash commands, and reference docs — and merges **six** hook registrations into your `~/.claude/settings.json` (five for `track-changes`, one for `verified-import`; `tc-polish` owns no hook, and `/tc edits` is a subcommand).
...
You are reading the track-changes v9.13.3 bootstrap manifest. Follow these steps precisely.

### Step 1: Check for existing installation

Read `~/.claude/skills/track-changes/VERSION`. If the file exists, parse the first line as a version number. Compare it to `9.13.3` using semantic-version ordering (compare major, then minor, then patch as integers; treat a missing component as 0, so a two-part `2.0` is `2.0.0`).

- **Equal to 9.13.3** → "track-changes v9.13.3 is already installed. No update needed." **STOP.**
- **Higher than 9.13.3** → "track-changes v[installed] is newer than this bootstrap (9.13.3). Aborting — will not downgrade." **STOP.** (Note: 9.13.3 is NEWER than 9.9.x — compare the minor as an INTEGER, so 10 > 9.)
- **Lower than 9.13.3** (e.g. v9.9.3, v9.9.2, v9.9.1, v9.9.0, v9.8.x, v9.7.x, v9.6.x, v9.5.x, v9.4.x, v9.3.x, v9.2.x, v9.1.x, v9.0.x, v8.x, v7.0.x, v6.0.x, v5.0.x, v4.x, v3.0.x, or any v1.x or v2.x) → "Updating track-changes from v[installed] to v9.13.3." Proceed to Step 2. The download in Step 2 overwrites every skill file in place (and, from a pre-v5 install, adds the `tc-polish` skill), so the upgrade is a clean replacement (the upgrade also re-merges settings.json, laying down the single combined PreToolUse matcher group); existing `<mark>…</mark><sup>N</sup>` content in your documents is fully backward-compatible and is left untouched (the mark grammar is unchanged — no document migration is needed). **Upgrading from v4.0.x?** The per-file activation key was renamed `track-changes:` → `tc-track:` in v4.1 (the old key is a reserved Quarto YAML field that breaks `.qmd`/`.md` renders); swap any old keys to `tc-track:`. **v6 is behavior-changing:** `/draft` is now **user-only** — the AI can no longer suspend its own tracking (the suspension sentinel is written solely by the `UserPromptSubmit` hook on *your* `/draft` prompt). Everything the AI writes into a tracked deliverable is tracked; large new blocks use the new whole-region insertion (`::: {.tc-region …}` / `\tcregion`). No document migration — the mark grammar is backward-compatible. **Upgrading from v6?** The bare `/polish` and `/import` slash aliases are retired — use `/tc polish` and `/tc import` instead. The `polish` skill directory is renamed to `tc-polish` (the upgrade removes the old `~/.claude/skills/polish/` directory and `~/.claude/commands/import.md` if present). No document migration. **Upgrading from v7?** v8 adds the bundled `decap` editor pre-clean tool under `~/.claude/skills/track-changes/tools/`; no behavior change to tracking, the mark grammar, or the hook suite. **Upgrading from v8.0/v8.1?** 8.2.0 adds the import-fidelity coverage gate (`tc_core/coverage.py` + `tc_coverage_audit.py`, wired into the verified-import hook) and the `/tc coverage` audit command; existing import behavior is otherwise unchanged, hook count stays 6, no document migration. **Upgrading from v8.x?** v9.0.0 adds the source-validation discipline: `/tc source` staging, the `sourced` provenance value with `tc-src` bindings, mechanical gray-excerpt verification in the track-changes hook, the `/tc manifest` evidence file, the `annotate_source_pdf.py` tool, and LaTeX `tcverbatim` + the optional `tcregion` `src` argument. Existing tracking, imports, and polish are unchanged; the mark grammar is backward-compatible (absent provenance/`tc-src` reads as authored — the new optional `tcregion` `src` arg leaves every v6/v7 parse untouched); hook count stays 6; no document migration. **Upgrading from v9.0?** 9.1.0 tightens the source gate — a `sourced` region must now carry a reader-facing citation (the exact key when staged by `@citekey`) and can no longer land without its verified excerpt; no document migration, hook count stays 6. Existing sourced regions keep working; editing one now requires keeping its citation. **Upgrading from v9.1.0?** 9.1.1 lets a gray excerpt carry contiguous source context with the load-bearing sentence marked (`[…]{.tc-src-key}` / `\tcsrckey{…}`); additive and presentational, no gate change, no migration. **Upgrading from v9.1.x?** 9.2.0 adds web-source citation: `/tc source <url>` (or `/tc source @webkey`, a bib `@online`/`@misc` entry with `url`+`urldate`) captures a dated snapshot into the private `validation/sources/` folder and verifies gray excerpts against it, just like a file source; new shared lib `tc_core/websource.py` and a `.html` branch in `sourcetext.py`. No gate change, no new hooks (stays 6), no document migration. **Upgrading from v9.2.x?** 9.3.0 completes the sourced-region lifecycle: `/tc accept`/`/tc reject` of a `sourced`/`transcript` region now also removes its paired gray `.tc-verbatim` scaffolding block, and the human-facing evidence is generated automatically (the manifest refreshes on each sourced write; the annotated-PDF twin regenerates on accept — both best-effort, never blocking). Behavior-changing for resolution, but backward-compatible grammar, no new hooks (stays 6), no document migration. **Upgrading from v9.3.x?** 9.4.0 adds paragraph continuity for region acceptance: a region may carry `tc-join="prev"|"next"` (LaTeX: a 4th optional `egin{tcregion}{N}[prov][src][join]`) to rejoin an adjacent paragraph on `/tc accept` instead of leaving a standalone block, and `/tc accept` now emits an advisory orphan-paragraph warning. Additive/opt-in; absent the attribute, resolution is unchanged; back-compat grammar, no new hooks (stays 6), no migration. **Upgrading from v9.4.x?** 9.5.0 hardens web-source capture: the headless render waits for JavaScript content (`--virtual-time-budget`), a thin/empty snapshot now gives loud save-then-source guidance instead of staging a silent shell, a fail-closed capture leaves no junk file, and SSL/cert fetch errors report plainly. Additive/robustness; no gate, grammar, or hook change (stays 6); no migration. **Upgrading from v9.5.0?** 9.5.1 is a matcher bug fix: the gray-block pairing now recognizes an attribute-free `::: tc-verbatim` div (bare-class Pandoc form), not only `::: {.tc-verbatim …}`, so `/tc accept`/`reject` auto-removes a bare gray block too. Matcher-only; no behavior, grammar, or hook change (6); no migration. **Upgrading from v9.5.x?** 9.6.0 adds a tc-polish cumulative baseline: `/tc polish` auto-resolves its git baseline to *edits since the last polish* (precedence: `--baseline-ref` → pinned checkpoint → last `Accept polish marks` commit → pre-edit-streak → HEAD; patterns configurable via a repo-tracked `.polish-baselines.json`), plus a region-aware `next_mark_n` so a polish mark can't collide with a pending `.tc-region`. tc-polish only; no track-changes hook/grammar/accept change; no new hooks (6); no migration. **Upgrading from v9.6-9.8.x?** 9.9.0 adds a new track-changes subcommand, `/tc edits [file]`, for the review loop where you edit the document yourself instead of dictating corrections: it diffs against an automatic per-user snapshot of the file as the AI last left it (captured by the existing PostToolUse hook, `/draft` writes included) and reports the edited spans, tc-polish's scope over them, your project linter's findings inside them (repo-local `.tc-edits.json`), any tracked region an edit overlaps, and the next mark number. It never edits a document. Additive: no mark-grammar change, no change to any existing resolution path, **no new hook (the count stays 6)**, no document migration — four new files in the track-changes skill (`lib/tc_edits.py`, `reference/tc-edits.md`, `lib/tc_core/marknum.py`, `lib/tc_core/snapshot.py`) and the new `/tc edits` subcommand. **Upgrading from v9.9.0?** 9.9.1 is a packaging fix: 9.9.0's manifests listed only the two skill-level files and omitted both new `tc_core` modules, so a 9.9.0 install left `/tc edits` raising `ImportError` on `marknum` and the PostToolUse snapshot silently storing nothing. Manifest-only; no source, behavior, grammar, or hook change (6); no migration. **Upgrading from v9.9.1?** 9.9.2 makes the deployment paths verify what they actually produced: a new `lib/tc_selfcheck.py` imports every module named in `tc_core.MODULES` (a declaration that now ships inside the package) plus every `*.py` deployed across the three skills, `install.sh` fails the install rather than reporting success over a broken tree, and Step 4 above runs it in place of an enumerated list of file-existence reads. Additive; no source-behavior, grammar, or hook change (6); no migration. **Upgrading from v9.9.2?** 9.9.3 addresses the frictions from `/tc edits`'s first real use: the report now prints each span's **before/after text** (it previously gave line numbers only, so it could be reviewed only by a session that still remembered the old text), bare `/tc edits` reports every file with a delta instead of erroring, and the snapshot store gains read-only `diff`/`show`. Additive; no new module, grammar, hook (6), or migration. **Upgrading from v9.9.x?** 9.10.0 adds an **accept-time provenance check**: when you `/tc accept` a green (`sourced`/`transcript`) region, content it gained since the write gate saw it with no basis in its gray excerpt is **named before accept removes the provenance claim**, and recorded in `.tc-history.md`. It is a conservative flag, not a proof, and with no durable audit record for that region it says "not checked" rather than passing silently. Also: accepting a region you emptied by hand no longer leaves stray blank lines, a resolved region's body now reaches the audit log, and `/tc edits` reports what changed **inside** a region rather than only that a span overlapped one. **Region atomicity is unchanged** — `/tc accept`/`/tc reject` remain all-or-nothing; to refuse part of a region, delete it before accepting. No new files, no new hook (6), back-compatible grammar, **no document migration**. **Upgrading from v9.10.0?** 9.11.0 changes what an install *does to files already on disk*: until now `install.sh` only ever copied, so an installed tree was the union of every version ever installed over it — a measured upgrade left five files behind that the current version does not ship, including two executable hook scripts from the superseded v2 architecture. An install now **moves unrecognized `.py`/`.sh` files out of `lib/` and `hooks/`** into `<skill>/.stale/<UTC-timestamp>/`, preserving their relative paths, and names every one on stdout. Nothing is deleted — the quarantine is recoverable, and you can remove `.stale/` yourself once you have looked at it. **Your data is not touched:** `state/` (the `/tc edits` snapshot store and mark cache) and `validation/` (source snapshots, manifests, dated web captures) are never visited, and `reference/`, `tools/` and `commands/` are left alone in this release. If you keep your own file under a skill's `lib/` or `hooks/`, it will be quarantined — move it elsewhere first. **This applies to `install.sh` only.** The curl path you are reading now still overwrites without pruning, so a bootstrap upgrade does not clean an accumulated tree; run `bash install.sh` from a source checkout if you want that. No source-behavior, grammar, or hook change (6); no migration. **Upgrading from v9.11.0?** 9.11.1 corrects a line of `/tc edits` output that said the opposite of what it meant. The `polish:` line is renamed **`protected:`** — which is all it ever reported, namely jargon/code/math tokens the AI must *leave alone* — because its empty branch printed "nothing flagged in the edited text", meaning *no un-correctable tokens* while reading as *no errors found*. **`protected: none` means nothing needed protecting, never that nothing needed fixing.** The pointer to "`/tc polish` for the full editorial pass" is gone with it: in that flow the editorial pass is the AI reading those spans *now*, so it sent you away at the moment the work should start. The report now ends with `NOT PROOFREAD: nothing above has been checked for grammar, dropped words, or sense` whenever there is prose to read — everything above it is mechanical. Reporting only: no grammar, hook (6), or resolution change, and no migration. **Upgrading from v9.11.1?** 9.11.2 is documentation, and it documents the most useful thing about reviewing AI work that was previously written down nowhere a user would look: **a region is atomic, but its body is editable before you resolve it.** Edit it, commit, then `/tc accept N` — which keeps the body *as you left it*, because `accept` never inspected who wrote it. **Deleting part of a region is how you reject that part**, which is why there is no partial-reject command. One ordering rule comes with it: ask for polish *after* the accept, never before — inside a region body an AI edit needs no highlight and lands invisibly, whereas once the wrapper is gone an unmarked change to your prose is refused. Also fixed: `reference/tc-edits.md` had documented a `resolve` subcommand that was withdrawn before it ever shipped, so an installed skill was carrying instructions for a command that does not exist. No behaviour change, no grammar or hook change (6), no migration. **Upgrading from v9.11.2?** 9.12.0 closes a blind spot the 9.11.2 documentation had to write around. Content the AI writes **inside** a region body carries no highlight — region lines are already covered by the region's own number — so nothing in the document shows it, and if you had edited that body yourself, `/tc accept` would keep the AI's wording as your own prose. Now every such edit is recorded in `.tc-history.md` under `region-body:`, and **`/tc accept N` tells you** when the region you are accepting had its body modified after it was written. **It is a notice, not a refusal:** the exit code is unchanged and Claude stays free to revise a region it wrote, which is how a draft improves before you see it. Absence of a note means *not recorded*, never "clean" — a region created before 9.12.0 has no entries. The standing advice is unchanged: edit, commit, accept, *then* ask for polish. Additive: no new files, no new hook (6), no grammar change, no migration. **Upgrading from v9.12.0? READ THIS ONE.** 9.13.0 is the first release in which **track-changes creates git commits in your repository.** The reason: 8.1.0 requires resolution to run on *committed content*, and that was mistaken for requiring *you* to type `git commit` — costing two manual commits per review round, at the two moments you are trying to finish. Now `/tc edits <file>` commits your edits and accepts the regions you edited (you reviewed them by editing them), and `/tc accept`/`/tc reject` commit the file instead of refusing. **Only ever the file you named** — `git commit -- <path>`, never `-a` — so other dirty or staged work in the same repository is untouched, and every commit is announced. A pre-commit hook that refuses is a failure: it is reported and **nothing is resolved**; `--no-verify` is never used. **Claude's corrections are still never auto-accepted** — the round stops and hands them to you, and untouched regions and pending inline marks stay pending. One thing you lose deliberately: the old gate told you to commit, *review the diff*, then re-run, and that middle step is now skipped (you review when you edit and again when you rule on the marks). Prefer the old behaviour? `TC_NO_AUTOCOMMIT=1` restores the refusal in full. No grammar change, no new hook (6), no migration. **Upgrading from v9.13.0?** 9.13.1 is bookkeeping: two items carried as open defects since 9.9.3 are closed on evidence (neither was a defect), and `reference/tc-edits.md` gains one line — ask for proofreading *after* running `/tc edits`, not before, or your edits and Claude's corrections share a commit instead of getting one each. No behaviour change. **Upgrading from v9.13.1?** 9.13.2 changes the landing page only — it had reached seventeen equally-weighted sections on one page, so it now has three clearly separated parts with the sections beneath them. Nothing was moved, added, or rewritten, and no skill file changed.
- **Does not exist** → "Installing track-changes v9.13.3." Proceed to Step 2.
...
**This is not a general prune, and this path does not have one.** 9.13.3 gave `install.sh` the ability to quarantine files it did not deploy, derived from a record the copy step produces as it runs. The curl path has no such record — it downloads an enumerated list and cannot tell an obsolete file from one it simply never knew about — so a tree upgraded this way keeps accumulating. The two deletions above stay hardcoded because they are two known, named artifacts, not a mechanism. Report an unexpected file under `lib/` or `hooks/` rather than guessing at it.
...
1. Read `~/.claude/skills/track-changes/VERSION` → first line must be `9.13.3`.
...
4. **Run the self-check (9.9.2) — this is the primary verification gate:**

   ```
   python ~/.claude/skills/track-changes/lib/tc_selfcheck.py
   ```

   (Use whichever of `python3`, `python`, or `py -3` works on this machine.) It must exit **0** and print `tc_selfcheck: OK - N module(s) present and importable`. It imports every module named in `tc_core.MODULES` — the package's own declaration of its contents, which ships with it — plus every `*.py` actually deployed across the three skills, so it catches both a module that was never downloaded and one that cannot load. Any non-zero exit names the offending module and its reason; **report that verbatim and do not claim success.** If the self-check reports a module as "declared in tc_core.MODULES but NOT DEPLOYED", the download in Step 2 was incomplete — re-fetch that file.

   *Why this replaced an enumerated list of file-existence reads:* 9.9.0 added two `tc_core` modules to the package and to no manifest, including this one. An enumerated verification list has exactly the same blind spot as an enumerated download list — a file in neither is invisible to both — so the check is now driven by a declaration that travels with the package.
...
11. Read `~/.claude/skills/track-changes/lib/tc_edits.py` → must exist (the `/tc edits` backend, 9.9.0).
12. Read `~/.claude/commands/draft.md`, `~/.claude/commands/tc.md` → both must exist. (`tc-polish` has no command file — `/tc polish` invokes it via the `/tc` dispatcher, and `/tc edits` is a track-changes subcommand.) `/import` is retired — use `/tc import`.)
13. Read `~/.claude/settings.json`, parse as JSON, confirm `.hooks.PreToolUse` contains a **single matcher group** whose `hooks` array holds BOTH a command containing `verified-import/hooks/pre_tool_use.py` AND a command containing `track-changes/hooks/pre_tool_use.py`, with the **verified-import entry stacked before** the track-changes entry (same group → sequential execution; not two separate groups). (`tc-polish` adds no hook, and `/tc edits` is a subcommand, so the count stays six.)
14. Read `~/.claude/skills/track-changes/tools/decap.py` → must begin with `#!/usr/bin/env python3` (bundled dictation pre-clean filter).
...
> **track-changes v9.13.3 installed successfully — three skills + shared `tc_core` + bundled tools.**
>
> **`track-changes`** (`~/.claude/skills/track-changes/`): always-on mark-tracking. SKILL.md, VERSION, settings-patch.json, hooks/ (5 sh + 2 py), lib/ (7 py + 6 sh + 1 sty + the shared `tc_core` package — now 12 modules incl. `sourcetext.py`/`srcstage.py`/`cite.py`/`websource.py`/**`snapshot.py`**/**`marknum.py`**), reference/ (highlight-syntax.md, latex.md, quarto-notes.md, **digest.md**, **tc-edits.md**, tc-clean.css, tc-clean.js), tools/ (decap.py, decap_protect.txt, **annotate_source_pdf.py**).
>
...
- `skill/lib/tc_edits.py` → `~/.claude/skills/track-changes/lib/tc_edits.py`
- `skill/lib/tc_selfcheck.py` → `~/.claude/skills/track-changes/lib/tc_selfcheck.py`
...
- `skill/lib/tc_core/marknum.py` → `~/.claude/skills/track-changes/lib/tc_core/marknum.py`
- `skill/lib/tc_core/snapshot.py` → `~/.claude/skills/track-changes/lib/tc_core/snapshot.py`
...
- `skill/reference/tc-edits.md` → `~/.claude/skills/track-changes/reference/tc-edits.md`
...
*(No `import.md` — `/tc import` dispatches via the `/tc` command. No `tc-polish.md` — `/tc polish` dispatches via the `/tc` command, as does `/tc edits`.)*

---

## v9.9.0 Changelog Summary

**v9.9.0 (`/tc edits` Phase A — minor, additive).** A new `/tc edits` subcommand of `track-changes` itself, for the review loop where the author edits the document directly rather than dictating corrections — a loop the protocol already described in prose but had no tooling for. `/tc edits <file>` diffs the file against a snapshot of *the file as the AI last left it* and reports: the edited line spans (each classified `content` or `whitespace-only`, so on-save formatter noise stays visibly separate), tc-polish's dictated scope over just those spans, the project linter's findings **inside** them, any tracked region an edit overlaps, and the next free mark number. It **never edits the document** — the model then emits corrections as ordinary marks through the always-on gate, so the author's own text stays clean and only the AI's corrections are marked.

The baseline is a new per-user snapshot store (`tc_core/snapshot.py`), written by the existing PostToolUse hook **before** its `/draft` early return, so it captures every AI write including drafted ones — git cannot separate the author's edits from the AI's in one dirty tree, and the audit log is blind to `/draft`. Three generations per file, which also makes the store an undo buffer (`/tc edits snapshots|restore`). Cold start (no snapshot) falls back to git HEAD and says so. An empty diff while the file is uncommitted, or while the previous generation differs, warns that an AI write may have absorbed the edits and names `--gen 1`.

Convention linting is a **project-supplied** command configured in a repo-local `.tc-edits.json` — the skill is generic and knows nothing about any project's rules. Executed as an argument list with no shell, with the argv printed before it runs; `--no-lint` / `TC_EDITS_NO_LINT=1` opt out. Also: `tc_core/marknum.py` is now the single mark-number allocator shared by tc-polish and `/tc edits`, `polish_engine.analyze` gains `--baseline-file` (a non-git baseline, mutually exclusive with `--baseline-ref`), and `tc_core.activation` gains `tc_should_track_inherent` (activation ignoring the per-turn `/draft` suspension).

Phase A is **clean-prose scope**: a region overlapping an edited span is reported, never resolved. Region splitting, partial accept, and provenance retyping are Phase B. No mark-grammar change, no new hook (count stays 6), no change to any existing resolution path, no document migration.
