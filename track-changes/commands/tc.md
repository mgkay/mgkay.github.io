---
description: track-changes unified command (draft / enable / disable / mark / migrate / status / list / accept / reject / edits / coverage / source / manifest / help / import / polish)
allowed-tools: Bash(bash:*)
argument-hint: "draft|enable|disable|mark|migrate|status|list|accept|reject|accept-all|reject-all|edits|coverage|source|manifest|import|polish|help [args]"
---
...
- `/tc edits [<file>]` — (9.9.0) after YOU hand-edit a tracked file, report what changed **since the AI last wrote**: the edited line spans with a real diff (each classified `content` or `whitespace-only`, so an on-save formatter's reflow stays visibly separate), tc-polish's scope over just those spans, the project linter's findings inside them, any tracked region an edit overlaps, and the next free mark number. The baseline is an automatic per-write snapshot, **not git** — during a review session the tree is dirty with both authors' edits and git cannot separate them. **Reports only; never edits the document**, and your text stays clean: only the AI's corrections to it get marks. Omit `<file>` to report every file with a delta. Also `/tc edits snapshots|diff|show|restore <file>`.
